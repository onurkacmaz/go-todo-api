...
func (t Token) Create(u User) Token {
	now := time.Now()
	...
	r, err := database.Instance().Exec(`INSERT INTO tokens (user_id, token, expired_at, created_at) VALUES (?,?,?,?)`, u.Id, t.Token, t.ExpiredAt, t.CreatedAt)
	if err != nil {
